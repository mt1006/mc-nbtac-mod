...
public class SimpleStringReader
{
	...
	public SimpleStringReader(String str, boolean jsonMode)
	{
		this.str = str;
		this.len = str.length();
	...
	private void readNbtString(Consumer<StringResults> setStr, boolean allowDot)
	{
		...
		while (true)
		{
			...
			{
				if (startPos == pos) { throw new ReaderException(); }
				setStr.accept(new StringResults(str.substring(startPos, pos), true, '\0'));
				return;
			}
			pos++;
		}
	...
	private StringResults readQuotedString(char quote, boolean requireClosed)
	{
		...
		while (true)
		{
			if (pos == len)
			{
				...
				return new StringResults(builder.toString(), false, quote);
			}
			...
			{
				if (ch == quote) { return new StringResults(builder.toString(), true, quote); }
	...
	public static class StringResults
	{
		...
		public final char quoteType;

		private StringResults(String str, boolean closed, char quoteType)
		{
			this.str = str;
			this.closed = closed;
