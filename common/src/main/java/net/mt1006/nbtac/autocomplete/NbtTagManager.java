...
public class NbtTagManager
{
	...
	{
		return name != null ? load(str, get(name), builder, suggestPath, name, null) : Suggestions.empty();
	}
	...
	public static CompletableFuture<Suggestions> load(String str, @Nullable NbtTagMap tagMap, SuggestionsBuilder builder,
													  boolean suggestPath, @Nullable String rootTagName, @Nullable NbtTag rootTag)
	{
		if (!Loader.finished)
		{
			new RawSuggestion("", "[suggestions not loaded]").suggest(builder);
