...
public class CustomTagParser
{
	...
	{
		if (element instanceof ParsedCompound compound) { parseCompound(compound); }
		else if (element instanceof ParsedList list) { parseListOrArray(list, false); }
		else if (element instanceof ParsedArray array) { parseListOrArray(array, true); }
		else if (element instanceof ParsedPrimitive primitive) { reader.readNbtString(primitive::setFromReader); }
		else { throw new IllegalStateException("Unexpected value: " + element); }
	}
