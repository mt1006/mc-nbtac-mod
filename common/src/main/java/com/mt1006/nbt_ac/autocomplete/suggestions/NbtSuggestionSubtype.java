...
public enum NbtSuggestionSubtype
{
											 ...
											 @Nullable String data, CustomTagParser.Type parserType)
	{
		switch (this)
		{
			...
			case ATLAS:
				suggestionList.clear();
				Collection<Identifier> atlases = ((AtlasManagerFields)Minecraft.getInstance().getAtlasManager()).getAtlasById().keySet();
				atlases.forEach((id) -> suggestionList.add(new IdSuggestion(id, "[#atlas]", parserType)));
				return true;

			case SPRITE:
				Identifier atlasId = data != null ? Identifier.tryParse(data) : null;
				if (atlasId == null) { break; }

				Map<Identifier, AtlasEntryFields> atlasById = ((AtlasManagerFields)Minecraft.getInstance().getAtlasManager()).getAtlasById();
				AtlasEntryFields atlasEntry = atlasById.get(atlasId);
				if (atlasEntry == null) { break; }

				suggestionList.clear();
				Collection<Identifier> sprites = ((TextureAtlasFields)atlasEntry.getAtlas()).getTexturesByName().keySet();
