...
public class Disassembly
{
	...
	private static final String METHOD_LOAD_ENTITY_SIGNATURE = "(L" + CompoundNBT.class.getName().replace('.', '/') + ";)V";
	private static final String METHOD_LOAD_BLOCK_ENTITY_SIGNATURE = "(L" +
			BlockState.class.getName().replace('.', '/') + ";L" + CompoundNBT.class.getName().replace('.', '/') + ";)V";
	private static final String COMPOUND_TAG_SIGNATURE = CompoundNBT.class.getName().replace('.', '/');
	private static final String LIST_TAG_SIGNATURE = ListNBT.class.getName().replace('.', '/');
	private static final String COMPOUND_TAG_ARG_SIGNATURE = CompoundNBT.class.getName();
	private static final String LIST_TAG_ARG_SIGNATURE = ListNBT.class.getName();
	...
	public static void init()
	{
		try
		{
			MethodNode methodNode = loadMethod(loadClass(TileEntity.class.getName()), null, METHOD_LOAD_BLOCK_ENTITY_SIGNATURE, true, false);
	...
	{
		disassemblyLoadMethod(Entity.class, null, METHOD_LOAD_ENTITY_SIGNATURE, clazz, arg);
	}

	public static void disassemblyBlockEntity(Class<?> clazz, NbtSuggestions arg) throws Exception
	{
		disassemblyLoadMethod(clazz, blockEntityLoadMethod, METHOD_LOAD_BLOCK_ENTITY_SIGNATURE, clazz, arg);
	}
	...
	public static class ValueTracker extends Interpreter<TrackedValue>
	{
		...
		private TrackedValue onListTagInvoke(MethodInsnNode methodInsn, List<? extends TrackedValue> values, BasicValue basicValue)
		{
			if (values.size() != 2 || values.get(0).type != TrackedValue.Type.LIST_TAG ||
					!methodInsn.desc.equals("(I)L" + CompoundNBT.class.getName().replace('.', '/') + ";")) // getCompound
			{
				return TrackedValue.unknown(basicValue);
			}
