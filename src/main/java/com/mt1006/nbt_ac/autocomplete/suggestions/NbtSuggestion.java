...
public class NbtSuggestion extends CustomSuggestion
{
	...
	public enum Type
	{
		...
		UNKNOWN((byte)-1),
		BOOLEAN((byte)-1),
		BYTE(TagType.TAG_BYTE),
		SHORT(TagType.TAG_SHORT),
		INT(TagType.TAG_INT),
		LONG(TagType.TAG_LONG),
		FLOAT(TagType.TAG_FLOAT),
		DOUBLE(TagType.TAG_DOUBLE),
		STRING(TagType.TAG_STRING),
		LIST(TagType.TAG_LIST),
		BYTE_ARRAY(TagType.TAG_BYTE_ARRAY),
		INT_ARRAY(TagType.TAG_INT_ARRAY),
		LONG_ARRAY(TagType.TAG_LONG_ARRAY),
		COMPOUND(TagType.TAG_COMPOUND),
		UUID((byte)-1);
		...
		public static void init()
		{
			...
			try
			{
				for (Method method : Disassembly.findClass(CompoundNBT.class.getName()).getMethods())
				{
		...
		private static Type fromMethodObject(Method method)
		{
			...
			{
				String getListSignature = "(Ljava/lang/String;I)L" + ListNBT.class.getName().replace('.', '/') + ";";
			...
			if (retTypeSignature.equals("L" + INBT.class.getName().replace('.', '/') + ";")) { return UNKNOWN; }
			if (retTypeSignature.equals("L" + CompoundNBT.class.getName().replace('.', '/') + ";")) { return COMPOUND; }
